...
def get_data():
    movie_data = pd.read_csv("/home/bhanukhetharpal/movie_data.csv")
...
def recommend_movies(title, data, combine, transform):
    ...
    sim_final = [ x for x in sim_scores if x[1]>0.20 ]
    if len(sim_final)<10:
        ans = len(sim_final)
    else:
        ans = 10
    sim_final = sim_final[1:ans]

    movie_indices = [i[0] for i in sim_final]
    ...
    match_scores = [int(i[1]*100) for i in sim_final]

    recommendation_data = pd.DataFrame(columns=['Movie_Id','Name','Genres','Match_Score'])

    recommendation_data['Movie_Id'] = movie_id
    recommendation_data['Name'] = movie_title
    recommendation_data['Genres'] = movie_genres
    recommendation_data['Match_Score'] = match_scores
